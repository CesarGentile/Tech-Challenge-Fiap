...
# Criando agora 3 abas
tab1, tab2, tab3 = st.tabs(["🤖 Preditor Analítico (IA)",
                           "📊 Dashboard de Insights Médicos", "📖 Dicionário de Dados"])
...
with tab1:
    ...
    st.markdown(
        "Insira os dados do paciente para estimar a categoria de peso baseada em Machine Learning.")
    ...
    with col1:
        ...
        weight = st.number_input(
            "Peso (kg)", min_value=30.0, max_value=250.0, value=70.0, step=0.1)
        family_hist = st.selectbox(
            "Histórico Familiar de Sobrepeso?", ["yes", "no"])

    with col2:
        favc = st.selectbox(
            "Consome alimentos calóricos (Fast Food)?", ["yes", "no"])
        fcvc = st.slider("Frequência de vegetais nas refeições (1-3)", 1, 3, 2)
        ncp = st.slider("Refeições principais por dia", 1, 4, 3)
        caec = st.selectbox("Come lanches entre as refeições?", [
    ...
    if st.button("Realizar Diagnóstico", type="primary"):
        ...
        st.success(
            f"### Categoria Prevista: **{prediction.replace('_', ' ')}**")
...
with tab2:
    ...
    st.markdown(
        "Insights derivados de dados para apoio a políticas de saúde e conscientização de pacientes.")
...
with tab3:
    ...
    st.markdown(
        "Abaixo estão as descrições de todas as variáveis utilizadas neste sistema, conforme a base original do estudo:")
